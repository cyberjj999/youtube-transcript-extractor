...
function extractTranscriptText() {
    ...
    let transcriptBody = document.querySelector(
        "#body > ytd-transcript-body-renderer"
    );
    if (transcriptBody != null) {
        let transcriptBodyList = document.querySelector(
            "#body > ytd-transcript-body-renderer"
        ).childNodes;
        ...
        for (var i = 0; i < transcriptBodyList.length; i++) {
            ...
            if (transcriptChild != undefined) {
                // check if child is defined
                var transcriptChildText = document.querySelectorAll(
                    "div.cues.style-scope.ytd-transcript-body-renderer > div"
                )[i];
    ...
    let menu = document
        ...
        .getElementsByTagName("yt-icon-button");
    for (var i = 0; i < menu.length; i++) {
        if (
            menu[i].className ==
